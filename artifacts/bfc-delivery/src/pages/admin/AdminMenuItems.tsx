...
function useItemOptionsKey(itemId: number | null) {
  return ["admin-item-options", itemId];
}
...
export default function AdminMenuItems() {
  ...
  // Add-category sidebar state
  const [addingCategory, setAddingCategory] = useState(false);
  const [newCategoryName, setNewCategoryName] = useState("");

  useEffect(() => {
    if (urlRestaurantId) setSelectedRestaurantId(parseInt(urlRestaurantId));
  }, [urlRestaurantId]);

  useEffect(() => { setSelectedCategory(null); }, [selectedRestaurantId]);
  ...
  const { data: categories = [], isLoading: isCatsLoading } = useQuery<RestaurantCategory[]>({
    queryKey: useCategoriesKey(selectedRestaurantId),
    queryFn: async () => {
      if (!selectedRestaurantId) return [];
      const res = await adminApi(`/admin/restaurants/${selectedRestaurantId}/categories`);
  ...
  // Existing options for the item being edited
  const { data: existingOptions = [] } = useQuery<MenuItemOption[]>({
    queryKey: useItemOptionsKey(editing?.id ?? null),
    queryFn: async () => {
      if (!editing) return [];
      const res = await adminApi(`/admin/menu-items/${editing.id}/options`);
  ...
  const addCategoryMutation = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: useCategoriesKey(selectedRestaurantId) });
  ...
  const deleteCategoryMutation = useMutation({
    mutationFn: (cat: RestaurantCategory) =>
      adminApi(`/admin/restaurants/${selectedRestaurantId}/categories/${cat.id}`, { method: "DELETE" }),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: useCategoriesKey(selectedRestaurantId) });
  ...
  const saveMutation = useMutation({
    mutationFn: async () => {
      ...
      let itemId: number;
      if (editing) {
        const res = await adminApi(`/admin/menu-items/${editing.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
        if (!res.ok) throw new Error("Failed to update");
        const item = await res.json();
        itemId = item.id;
      } else {
        const res = await adminApi("/admin/menu-items", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
        if (!res.ok) throw new Error("Failed to create");
        const item = await res.json();
        itemId = item.id;
      }
      // Save pending options (for new items)
      for (const opt of pendingOptions) {
        await adminApi(`/admin/menu-items/${itemId}/options`, {
    ...
    onSuccess: () => {
      ...
      if (editing) qc.invalidateQueries({ queryKey: useItemOptionsKey(editing.id) });
      setDialogOpen(false);
  ...
  const toggleMutation = useMutation({
    mutationFn: (item: MenuItem) =>
      adminApi(`/admin/menu-items/${item.id}`, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ isAvailable: !item.isAvailable }) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: getGetRestaurantMenuQueryKey(selectedRestaurantId ?? 0) }),
  });
  ...
  const addExistingOptionMutation = useMutation({
    mutationFn: (opt: { name: string; price: number }) =>
      adminApi(`/admin/menu-items/${editing!.id}/options`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(opt) }),
    onSuccess: () => { qc.invalidateQueries({ queryKey: useItemOptionsKey(editing?.id ?? null) }); setNewOption(EMPTY_OPTION); },
    onError: () => toast({ title: "Failed to add option", variant: "destructive" }),
  });

  const deleteOptionMutation = useMutation({
    mutationFn: (optionId: number) => adminApi(`/admin/menu-item-options/${optionId}`, { method: "DELETE" }),
    onSuccess: () => qc.invalidateQueries({ queryKey: useItemOptionsKey(editing?.id ?? null) }),
        ...
        <div className="w-64 shrink-0 border-r border-zinc-200 flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto flex flex-col">
            {!selectedRestaurantId ? (
              <div className="p-4 text-sm text-zinc-400 text-center mt-4">Select a restaurant to view categories.</div>
            ) : isCatsLoading ? (
                ...
                {addingCategory ? (
                  <div className="flex gap-1">
                    <Input
                      ...
                      onKeyDown={e => { if (e.key === "Enter") addCategoryMutation.mutate(newCategoryName); if (e.key === "Escape") { setAddingCategory(false); setNewCategoryName(""); } }}
