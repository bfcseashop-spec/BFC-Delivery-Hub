...
import {
  ...
  ArrowLeft, Store, ChevronRight, Award,
} from "lucide-react";
...
export function PartnerLayout({ children, title }: { children: React.ReactNode; title: string }) {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!r.ok) return null;
      return r.json() as Promise<{ partner: { name: string; businessName: string; commissionRate: number }; restaurant: { name: string; id: number } | null }>;
    },
    enabled: !!partnerId,
  });
        ...
        <div className="p-4 border-t border-zinc-200 dark:border-zinc-800 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-[10px] text-muted-foreground">{data?.partner.commissionRate ?? 15}% commission</p>
            </div>
          </div>
        </div>
      </aside>
